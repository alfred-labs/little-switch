...
extension OpenAIResponsesNativeNamespacingTests {
    ...
    @Test("Exchange keys are stable and unencodable exchanges cannot collapse")
    func exchangeKeys() throws {
        ...
        #expect(firstKey != nil)
        // A value JSON cannot represent yields no key, so the exchange
        // never enters a run of repeated pairs.
