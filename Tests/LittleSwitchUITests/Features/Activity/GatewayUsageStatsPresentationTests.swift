...
struct GatewayUsageStatsPresentationTests {
    ...
    func aggregatePeriod() {
        ...
        #expect(period.accessibilityValue == "30 days, \(1_416_000.formatted(.number)) tokens")
        #expect(
            period.metrics == [
                metric("input-tokens", "Input tokens", "1.2M", leading: false),
                metric("cached-tokens", "Cached tokens", "200K", leading: false),
    ...
    func selectedPeriod() {
        ...
        #expect(yesterday.accessibilityValue == "Aug 28, \(1_210_000.formatted(.number)) tokens")
        #expect(yesterday.metrics.map(\.value) == ["1M", "200K", "10K", "100", "10%", "2"])
        #expect(today.label == "Today")
        #expect(today.tokenTotal == "206K")
        #expect(today.metrics.map(\.value) == ["200K", "0", "6K", "39", "26%", "3"])
        #expect(stats.period(at: nil).metrics.map(\.value) == ["1.2M", "200K", "16K", "139", "14%", "5"])
    }
    ...
    func estimatedPeriods() {
        ...
        #expect(
            stats.period(at: nil).accessibilityValue
                == "30 days, \(1_106.formatted(.number)) tokens, includes estimates"
        )
        #expect(stats.period(at: 28).metrics.map(\.value) == ["1K", "50", "10", "0", "0%", "0"])
        #expect(stats.period(at: 28).accessibilityValue.hasSuffix("includes estimates"))
        #expect(stats.period(at: 29).metrics[0].title == "Input tokens")
        #expect(stats.period(at: 29).accessibilityValue == "Today, 6 tokens")
    }
    ...
    func saturatedPeriods() {
        ...
        #expect(period.tokenTotal == GatewayUsageFormat.tokenTotal(Int.max))
        #expect(period.accessibilityValue == "30 days, \(Int.max.formatted(.number)) tokens")
        #expect(period.metrics[3].value == Int.max.formatted(.number))
        #expect(period.metrics[4].value == "50%")
        #expect(period.metrics[5].value == Int.max.formatted(.number))
        #expect(
            period.metrics.prefix(3).map(\.value)
                == Array(repeating: GatewayUsageFormat.compactCount(Int.max), count: 3)
        )
    }
    ...
    func seriesLabels() {
        ...
        #expect(stats.period(at: 29).accessibilityValue == "Today, \(206_000.formatted(.number)) tokens")
    ...
    func dayLabelsFollowTheKey() {
        ...
        #expect(GatewayUsageStatsPresentation.dayLabel("2026-01-01") == "Jan 1")
        #expect(GatewayUsageStatsPresentation.dayLabel("2026-12-31") == "Dec 31")
        #expect(GatewayUsageStatsPresentation.dayLabel("2028-02-29") == "Feb 29")
    }
}

private func presentation(days: [GatewayUsageDay]) -> GatewayUsageStatsPresentation {
