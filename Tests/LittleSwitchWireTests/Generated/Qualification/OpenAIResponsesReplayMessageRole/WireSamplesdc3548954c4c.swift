...
enum WireSamplesdc3548954c4c {
    static let samples: [WireGeneratedSample] = [
        .init(
            name: "OpenAIResponsesReplayMessageRole.enum:assistant",
            input: """
                \"assistant\"
                """,
            expectedError: nil
        ) { json in
            return try OpenAIResponsesReplayMessageRole(wireJSON: json).wireJSON()
        },
        .init(
            name: "OpenAIResponsesReplayMessageRole.invalid-enum",
            input: """
                \"__wire_unknown__\"
                """,
