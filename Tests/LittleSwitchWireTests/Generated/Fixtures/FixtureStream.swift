...
public indirect enum FixtureStream: Sendable, WireCodable {
    case variant1(FixtureStreamVariant1)
    case variant2(FixtureStreamVariant2)

    public init(wireJSON: JSONValue) throws {
        // anyOf chooses the first valid source branch and preserves its full payload.
        if let value = try? Self.decode(FixtureStreamVariant1.self, from: wireJSON) {
            self = .variant1(value)
            return
        }
        if let value = try? Self.decode(FixtureStreamVariant2.self, from: wireJSON) {
            self = .variant2(value)
            return
        }
