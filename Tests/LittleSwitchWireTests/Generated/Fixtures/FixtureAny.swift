...
public indirect enum FixtureAny: Sendable, WireCodable {
    case variant1(FixtureAnyVariant1)
    case variant2(FixtureAnyVariant2)

    public init(wireJSON: JSONValue) throws {
        // anyOf chooses the first valid source branch and preserves its full payload.
        if let value = try? Self.decode(FixtureAnyVariant1.self, from: wireJSON) {
            self = .variant1(value)
            return
        }
        if let value = try? Self.decode(FixtureAnyVariant2.self, from: wireJSON) {
            self = .variant2(value)
            return
        }
